...
import os, base64, uuid, datetime as dt, json, requests
...
@app.get("/health")
def health():
    return {"ok": True, "ts": int(dt.datetime.utcnow().timestamp())}
...
def list_cases(user_id: str = Depends(get_current_user)):
    ...
    items = []
    for d in q:
        x = d.to_dict(); x["id"] = d.id; items.append(x)
    return {"cases": items}

@app.get("/cases/{case_id}")
def get_case(case_id: str, user_id: str = Depends(get_current_user)):
    doc = db.collection("cases").document(case_id).get()
    if not doc.exists: raise HTTPException(404, "Caso não encontrado.")
    data = doc.to_dict()
    if data.get("owner_uid") != user_id: raise HTTPException(403, "Acesso negado.")
    data["id"] = case_id
    return data

@app.post("/cases/{case_id}/evidences")
def add_evidence(case_id: str,
                 request: Request,
                 url: str = None,
                 headers: str = None,
    ...
    cdoc = cref.get()
    if not cdoc.exists: raise HTTPException(404, "Caso não encontrado.")
    if cdoc.to_dict().get("owner_uid") != user_id: raise HTTPException(403, "Acesso negado.")

    if not url and not headers and not file:
        raise HTTPException(422, "Informe ao menos um dos campos: url, headers ou file.")

    ev = {"added_at": dt.datetime.utcnow().isoformat()}
    if url: ev["url"] = url
    if headers:
        try:
    ...
    ref, _ = cref.collection("evidences").add(ev)
    ev["id"] = ref.id
    return ev
...
def canary(case_id: str, request: Request):
    cref = db.collection("cases").document(case_id)
    if not cref.get().exists: raise HTTPException(404, "Caso não encontrado.")
    ip = request.headers.get("x-forwarded-for") or request.client.host
    ua = request.headers.get("user-agent", "N/A")
    cref.collection("canary_logs").add({
        "ip": ip, "user_agent": ua, "timestamp": dt.datetime.utcnow().isoformat()
    })
...
def osint_lookup(username: str, user_id: str = Depends(get_current_user)):
    ...
    try:
        ...
        r = requests.get(f"https://www.reddit.com/user/{username}/about.json", headers=h, timeout=5)
